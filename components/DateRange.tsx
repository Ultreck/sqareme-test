    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button className="flex border bg-white outline-0 justify-between md:px-3 px-1 py-2 rounded-lg items-center space-x-2 font-normal">
          ...
          {date?.from ? (
            <span className="text-[#71717A] flex text-xs md:text-sm items-center ">
              {handleFrom(date)} - {handleTo(date)}
            </span>
          ) : (
            <span className="text text-xs md:text-sm">
              Aug 12, 2025 - Aug 13, 2025
            </span>
          )}
